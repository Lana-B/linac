...
ur=uproot.pandas.iterate(filename, "PhaseSpace", ['X','Y','Ekine'])
mylist=list(ur)        
...
for i in range(file_num,file_num+1):
	...
	newdf=pd.DataFrame(newsample,columns=['X','Y','Ekine'])
	...
	newdfsub.to_root(f"/work/lb8075/PhaseSpaces/Generated_PhS2_Electrons_{field_size}/generatedElectron_PhS2_v4_perp_{field_size}_{i}_file{n_file}.root", key='PhaseSpace')
